    ...
    void enviar(MouseEvent event) {
        if(this.alumno==null){
            if(!validarContenido("^[A-Z][a-z]{3,20}$",this.introducirNombre.getText()) || !validarContenido("^[A-Z][a-z]+\\\\s[A-Z][a-z]+$",this.introducirApellidos.getText())||
                ...
                !this.introducirNota.getText().isEmpty()&& !validarContenido("(?:10|\\d(?:\\.\\d{1,2})?)",this.introducirNota.getText())||
                this.introducirCiclo.getValue()==null
            ){
                return;
            }
                ...
                ControllerGestion controllerGestion=new ControllerGestion();
                controllerGestion.recibirAlumno(alumnoCreado);
